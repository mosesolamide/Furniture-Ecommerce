            ...
            <ul className="flex flex-col md:flex-row flex-wrap gap-30 md:gap-12 w-full list-none p-0 m-0">
              {furnitureList.slice(0, size).map((item: FurnitureType): JSX.Element => (
                <li key={item.id} className="flex-1 min-w-[150px]">
                  <article
                    tabIndex={0} // Required for focus-visible
                    className="flex flex-col items-center justify-center relative group hover:cursor-pointer focus:outline-none"
                  >
                    {/* Product Image */}
                    <div className="w-50 h-50">
                      <img
                        src={item.imageUrl}
                        alt={item.name}
                        className="w-full h-full object-contain transition-all duration-300 ease-in-out transform translate-y-0 group-hover:translate-y-[-19%] group-focus-visible:translate-y-[-19%]"
                      />
                    </div>
                    ...
                    {/* Hidden "Add to Cart" Overlay (Shows on Hover/Tap) */}
                    <div
                      className="transition-all duration-300 ease-in-out transform h-55 md:h-40 lg:h-50
                                 ...
                                 group-focus-visible:scale-y-100 group-focus-visible:opacity-100
                    ...
                    >
                      <img
                        ...
                        className="bg-gray-900 p-2 rounded-full opacity-0 transition-all ease-in-out duration-300
                                   ...
                                   group-focus-visible:opacity-100 group-focus-visible:translate-y-30
                                   md:group-hover:translate-y-23 md:group-focus-visible:translate-y-23"
                      />
                    </div>
                  </article>
                </li>
